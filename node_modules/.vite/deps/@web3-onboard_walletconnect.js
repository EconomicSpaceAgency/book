...
import {
  u
} from "./chunk-GKF6NWNH.js";
import "./chunk-72KYRN3M.js";
import "./chunk-J62NGYXO.js";
import "./chunk-SCEK4EBZ.js";
import {
  require_joi_browser_min
} from "./chunk-QDIGJPMI.js";
import "./chunk-EL4GROKR.js";
import {
  __toESM,
  require_dist
...
// node_modules/@web3-onboard/walletconnect/dist/v1.js
...
function walletConnect(options) {
  ...
  const { bridge, qrcodeModalOptions, connectFirstChainId, handleUri } = options || {};
  console.warn(`Wallet Connect version 1 support has been deprecated by the WalletConnect team. Please consider using version 2. See docs for more details.`);
  if (!bridge) {
  ...
  return () => {
    return {
      label: "WalletConnect",
      getIcon: async () => (await import("./icon-3RSO2IAY.js")).default,
      getInterface: async ({ chains, EventEmitter }) => {
        const { StaticJsonRpcProvider } = await import("./lib-TY4Y7OSK.js");
        const { ProviderRpcError, ProviderRpcErrorCode } = await import("./dist-VOXQ6GJD.js");
        const { default: WalletConnect } = await import("./esm-O36PM65J.js");
        let QRCodeModal = await import("./cjs-OSRBDQC4.js");
        QRCodeModal = QRCodeModal.default || QRCodeModal;
        const { Subject, fromEvent } = await import("./esm5-L4ZNQYO5.js");
        const { takeUntil, take } = await import("./operators-BIYKMZNO.js");
...
var v1_default = walletConnect;

// node_modules/@web3-onboard/walletconnect/dist/v2.js
...
function walletConnect2(options) {
  ...
  return () => {
    return {
      ...
      getInterface: async ({ chains, EventEmitter, appMetadata }) => {
        const { ProviderRpcError, ProviderRpcErrorCode } = await import("./dist-VOXQ6GJD.js");
        const { default: EthereumProvider } = await import("./index.es-6DIF7J35.js");
        const { Subject, fromEvent } = await import("./esm5-L4ZNQYO5.js");
        const { takeUntil, take } = await import("./operators-BIYKMZNO.js");
        const getMetaData = () => {
          if (!appMetadata)
        ...
        const requiredMethodsSet = new Set(additionalRequiredMethods && Array.isArray(additionalRequiredMethods) ? [...additionalRequiredMethods, ...u] : u);
...
var wcOptions = import_joi.default.object({
  ...
  version: import_joi.default.number().valid(1, 2).optional(),
  bridge: import_joi.default.string().when("version", {
    is: 1,
  ...
  connectFirstChainId: import_joi.default.boolean().optional(),
  qrcodeModalOptions: import_joi.default.object({
    mobileLinks: import_joi.default.array().items(import_joi.default.string()).optional()
  }).optional(),
  projectId: import_joi.default.string().when("version", {
    is: 2,
    then: import_joi.default.required(),
    otherwise: import_joi.default.optional()
  }).messages({
    "any.required": `WalletConnect version 2 requires a projectId. Please visit https://cloud.walletconnect.com to get one.`
  }),
  dappUrl: import_joi.default.string().optional(),
...
function walletConnect3(options) {
  if (!options) {
  ...
  return options.version === 2 ? v2_default(options) : v1_default(options);
}
var dist_default = walletConnect3;
export {
